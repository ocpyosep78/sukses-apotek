...
foreach ($array_dinamic as $no => $data) { ?>
    <tr class="child<?= $key ?>" id="child<?= $key ?><?= $no ?>"><td></td><td>
        <table width=100% style="border-bottom: 1px dotted #ccc; margin-bottom: 5px; padding-bottom: 3px;">
            ...
            <tr><td>Range Juala:</td><td><input type=text name=awal<?= $key ?>[] value="<?= $data->jual_min ?>" id=awal<?= $key ?><?= $no ?> size=5 /> <div class="space"> s.d </div> <input type=text name=akhir<?= $key ?>[] value="<?= $data->jual_max ?>" id=akhir<?= $key ?><?= $no ?> size=5 /></td><td>Diskon:</td><td><input type=text name=d_persen<?= $key ?>[] id=d_persen<?= $key ?><?= $no ?> value="<?= $data->diskon_persen ?>" size=5 /> <div class="space"> (%) </div> <input type=text name=d_rupiah<?= $key ?>[] value="<?= rupiah($data->diskon_rupiah) ?>" id=d_rupiah<?= $key ?><?= $no ?> onblur="FormNum(this);" onfocus="javascript:this.value=currencyToNumber(this.value);" size=5 /></td></tr>
            <tr><td>Margin Non Resep:</td><td><input type=text name=margin_nr<?= $key ?>[] value="<?= $data->margin_non_resep ?>" id=margin_nr<?= $key ?><?= $no ?> size=5 /> <div class="space"> (%) </div><input style="background: #f4f4f4; border: none;" type=text id=margin_nr_rp<?= $key ?><?= $no ?> size=5 disabled /></td><td>Harga Jual Non Resep (Rp.):</td><td><input type=text name="hj_nonresep<?= $key ?>[]" value="<?= rupiah(ceil($data->hj_non_resep)) ?>" id="hj_nonresep_f<?= $key ?><?= $no ?>" size="15" onkeyup="FormNum(this);" onblur="set_margin('<?= $key ?>','<?= $no ?>');" /></td></tr>
            <tr><td>Margin Resep:</td><td><input type=text name=margin_r<?= $key ?>[] value="<?= $data->margin_resep ?>" id=margin_r<?= $key ?><?= $no ?> size=5 /> <div class="space"> (%) </div><input style="background: #f4f4f4; border: none;" type=text id=margin_r_rp<?= $key ?><?= $no ?> size=5 disabled /></td><td>Harga Jual Resep (Rp.):</td><td><input type=text name="hj_resep<?= $key ?>[]" value="<?= rupiah(ceil($data->hj_resep)) ?>" id="hj_resep_f<?= $key ?><?= $no ?>" onkeyup="FormNum(this);" onblur="set_margin('<?= $key ?>','<?= $no ?>');" size="15" /><img src="img/icons/delete.png" onclick="removeMe(this);" align=right /></td></tr>
        </table>
    </td></tr>
    <script type="text/javascript">
